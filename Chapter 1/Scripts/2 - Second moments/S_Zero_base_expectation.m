...
% normal zero-mean base and views on linear combinations of second non-central moments

clc; clear; close all;

%% Input parameters
mu_pri = [0; 0; 0]; % base expectation
...
theta_sig2_view = 0.5 * (inv(z_sig * sig2_pri * z_sig') - inv(eta_sig2_view));

%% Compute updated expectation and covariance
mu_pos = mu_pri;

z_sig_dag = PseudoInverse(z_sig, sig2_pri);
sig2_pos = sig2_pri + z_sig_dag' * (eta_sig2_view - z_sig * sig2_pri * z_sig') * z_sig_dag;
