function [theta_mu_view, theta_sig2_view, mu_pos, sig2_pos] = FitMREEq12MomentsN(z_mu, z_sig, eta_mu_view,  eta_sig2_view, mu_pri, sig2_pri, km)
% Compute the optimal Lagrangian multipliers, updated expectation and 
% covariance under normal base and equality views on linear combinations 
% of first moments and inequality views on second moments via numerical optimization
% update
%  INPUTS
%   z_mu            [matrix]   k_mu x n_ pick matrix for equality views on first moments
%   z_sig           [matrix]   k_sig x n_ pick matrix for equality views on second moments
...
[k_mu, n_] = size(z_mu);
k_sig = size(z_sig, 1);
k_ = k_mu + k_sig^2;  % total number of views 
