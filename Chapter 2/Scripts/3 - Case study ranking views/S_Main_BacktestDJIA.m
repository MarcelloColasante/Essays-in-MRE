% This script shows a systematic trading strategy, processing ranking 
% momentum signals in the Dow Jones equity market via MRE approach and
...
save MainBacktestDJIA.mat

pnl = [EW.pnl(2:end), MRE.pnl(2:end), CO.pnl(2:end), FEP.pnl(2:end), NS.pnl(2:end)];
m_pnl = mean(pnl);
s2_pnl = cov(pnl);
...
plot(EW.time, cumsum(EW.pnl), 'go-', 'linewidth', 3, 'markersize', 2);
plot(NS.time, cumsum(NS.pnl), 'ko-', 'linewidth', 3, 'markersize', 2);
grid on; datetick('x', 12); axis tight; 
xt = get(gca, 'XTick');
set(gca, 'FontSize', 16)
title('Cumulative PnL'); 
legend('MRE', 'FEP', 'Common', 'Equally weighted', 'Base', 'Location', 'NorthWest');
